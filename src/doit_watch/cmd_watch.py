...
execute tasks when file dependencies change"""
...
class Watch(DoitCmdBase):
    ...
    doc_purpose = "execute tasks when a dependency changes"
    doc_usage = "[TASK ...]"
    ...
    @staticmethod
    def _find_file_deps(tasks, sel_tasks) -> Set[str]:
        """find all file deps
        @param tasks (dict)
    ...
    @staticmethod
    def _dep_changed(watch_files: Set[str], started: float, targets: Set[str]) -> bool:
    ...
    def run_watch(self, params, args) -> Optional[None]:
        ...
        arun: Any = Run(task_loader=self.loader)
        ...
        watch_files = self._find_file_deps(
            arun.control.tasks, arun.control.selected_tasks
        )
        ...
        if not self._dep_changed(watch_files, started, arun.control.targets):
            ...
            class DoitWatchRun(FileModifyWatcher):
                def handle_event(self, event):
                    # print("FS EVENT -> {}".format(event))
                    sys.exit(result)

            file_watcher = DoitWatchRun(watch_files)
            # kick start watching process
            file_watcher.loop()

    def execute(self, params: Dict[str, Any], args: Any) -> int:
