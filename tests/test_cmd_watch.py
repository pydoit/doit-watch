...
from doit_watch import cmd_watch
...
class TestFindFileDeps(object):
    def find_deps(self, sel_tasks):
        ...
        return cmd_watch.Watch._find_file_deps(tasks, sel_tasks)
...
class TestDepChanged(object):
    def test_changed(self, dependency1):
        started = time.time()
        assert not cmd_watch.Watch._dep_changed([dependency1], started, [])
        assert cmd_watch.Watch._dep_changed([dependency1], started - 100, [])
        assert not cmd_watch.Watch._dep_changed(
            [dependency1], started - 100, [dependency1]
        )
...
class TestWatch(object):
    def test_invalid_args(self, dependency1, depfile_name):
        ...
        cmd = CmdFactory(cmd_watch.Watch, task_loader=task_loader)
    ...
    def test_run_callback(self, monkeypatch):
        ...
        monkeypatch.setattr(cmd_watch, "call", mock_cmd)

        # success
        result = []
        cmd_watch.Watch._run_callback(0, "success", "failure")
        assert "success" == result[0]

        # failure
        result = []
        cmd_watch.Watch._run_callback(3, "success", "failure")
        assert "failure" == result[0]

        # nothing executed
        result = []
        cmd_watch.Watch._run_callback(0, None, None)
        assert 0 == len(result)
        cmd_watch.Watch._run_callback(1, None, None)
        assert 0 == len(result)
    ...
    def test_run_wait(self, dependency1, target1, depfile_name):
        ...
        cmd = CmdFactory(cmd_watch.Watch, task_loader=FakeLoader([t1], depfile_name))

        run_wait_proc = Process(target=cmd.run_watch, args=(DefaultUpdate(), []))
        run_wait_proc.start()
    ...
    def test_execute(self, monkeypatch):
        ...
        monkeypatch.setattr(cmd_watch.Watch, "run_watch", fake_run)

        # after join raise exception to stop WATCH command
        original = cmd_watch.Process.join

        def join_interrupt(self):
            original(self)
            raise KeyboardInterrupt()

        monkeypatch.setattr(cmd_watch.Process, "join", join_interrupt)

        cmd = CmdFactory(cmd_watch.Watch)
